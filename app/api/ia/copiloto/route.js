...
export async function POST(req) {
  ...
  const lineas = (devs || []).map(d => {
    ...
    return `${d.nombre} (${d.alcaldia}): ${MXN(d.precio_min)}–${MXN(d.precio_max)}, ${d.rec_min}–${d.rec_max} rec, ${byDev[d.sku] || 0} disp., comisión ${Math.round((d.comision_broker || 0) * 100)}%, ${d.etapa}${creds ? ', créditos ' + creds : ''}.`;
  }).join('\n');
