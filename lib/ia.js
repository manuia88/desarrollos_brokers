...
export async function llamarIATools({ proveedor, apiKey, system, mensajes, herramientas, ejecutar, maxTokens = 600, maxVueltas = 5, model }) {
  ...
  const m = model || process.env.IA_MODEL || 'claude-3-5-haiku-latest';
  // Prompt caching: cachea el prefijo estable (tools + system) y, en cada vuelta,
  // el contexto acumulado (historial + resultados de herramientas) marcando el
  // último bloque del último mensaje. Escritura ~1.25x, lectura ~0.1x: baja el costo
  // real donde está el gasto (el contexto que se reenvía en cada round-trip y mensaje).
  const sysCache = [{ type: 'text', text: system, cache_control: { type: 'ephemeral' } }];
  const toolsCache = herramientas.map((h, i) => i === herramientas.length - 1 ? { ...h, cache_control: { type: 'ephemeral' } } : h);
  const conCache = (arr) => {
    if (!arr.length) return arr;
    const out = arr.slice();
    const ult = out[out.length - 1];
    let content = typeof ult.content === 'string' ? [{ type: 'text', text: ult.content }] : ult.content.map(b => ({ ...b }));
    if (content.length) content[content.length - 1] = { ...content[content.length - 1], cache_control: { type: 'ephemeral' } };
    out[out.length - 1] = { ...ult, content };
    return out;
  };
  const msgs = [...mensajes];
  for (let v = 0; v < maxVueltas; v++) {
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({ model: m, max_tokens: maxTokens, system: sysCache, messages: conCache(msgs), tools: toolsCache }),
    });
    if (!r.ok) throw new Error('IA (Anthropic) error ' + r.status);
    const j = await r.json();
    tin += (j.usage?.input_tokens || 0) + (j.usage?.cache_read_input_tokens || 0) + (j.usage?.cache_creation_input_tokens || 0);
