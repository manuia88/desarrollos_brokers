...
// Etiqueta legible para un medio: ambiente real > título propio > tipo.
...
export function etiquetaMedio(m) {
  ...
  if (t && !/^(captura de pantalla|screen ?shot|whats ?app image|img[-_ ]?\d|image$|imagen$|foto$|photo|dsc[-_ ]?\d|screen|sin t[ií]tulo|untitled|unnamed)/i.test(t)) return t;
