...
export function esquemaPago(precio, opts) {
  ...
  const sumaPct = mxn2(enganchePct + obraPct + escrituraPct);
  return {
    precio: p,
    ...
    sumaPct,
    // ¿los porcentajes suman ~100%? (tolerancia de medio punto)
    consistente: Math.abs(sumaPct - 1) < 0.005,
  };
}
...
// Bancos / fondos con tasa anual TÍPICA de referencia (editable en el cotizador).
// Fuente aproximada: comparadores públicos (Banxico/CONDUSEF). No es oferta vinculante.
export const BANCOS = [
  { nombre: 'Infonavit',   tasa: 9.00 },
  { nombre: 'FOVISSSTE',   tasa: 9.90 },
  { nombre: 'Citibanamex', tasa: 10.10 },
  { nombre: 'BBVA',        tasa: 10.20 },
  { nombre: 'Scotiabank',  tasa: 10.42 },
  { nombre: 'Banorte',     tasa: 10.90 },
  { nombre: 'HSBC',        tasa: 11.10 },
];
