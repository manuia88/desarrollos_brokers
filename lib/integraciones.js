...
// Base URL por ambiente y resolución de credenciales (cuenta o env global).
export const EB_BASE = { produccion: 'https://api.easybroker.com/v1', staging: 'https://api.stagingeb.com/v1' };
function ebCfg(cfg) {
  const key = cfg?.key || process.env.EASYBROKER_API_KEY || null;
  const base = cfg?.base || EB_BASE[cfg?.ambiente] || EB_BASE.produccion;
  return { key, base };
}
// Elige la credencial EB correcta para un (org, asesor) según el modo de la inmobiliaria.
// eb_modo='asesor' -> cada asesor usa la suya; 'org' -> todos usan la de la inmobiliaria.
export function elegirConexionEB(conexiones, { org_id, asesor_id, eb_modo }) {
  const list = (conexiones || []).filter(c => c.proveedor === 'easybroker' && c.activa && c.org_id === org_id);
  if (eb_modo === 'asesor' && asesor_id) {
    const c = list.find(x => x.scope === 'asesor' && x.asesor_id === asesor_id);
    if (c) return { key: c.api_key, ambiente: c.ambiente };
  }
  const org = list.find(x => x.scope === 'org');
  if (org) return { key: org.api_key, ambiente: org.ambiente };
  const any = list.find(x => x.scope === 'asesor' && x.asesor_id === asesor_id);
  if (any) return { key: any.api_key, ambiente: any.ambiente };
  return null; // sin conexión propia: el backend cae al env global (desarrollador/super)
}

// Valida una llave (con ambiente) haciendo una llamada de solo lectura.
export async function validarEB(key, ambiente = 'produccion') {
  const base = EB_BASE[ambiente] || EB_BASE.produccion;
  try { const r = await fetch(`${base}/properties?limit=1`, { headers: { 'X-Authorization': key, accept: 'application/json' } }); return r.ok; }
  catch { return false; }
}

// ---- ENTRADA: EasyBroker: traer listados ----
export async function fetchEasyBroker(page = 1, limit = 50, cfg) {
  const { key, base } = ebCfg(cfg);
  if (!key) return { skipped: true, propiedades: [] };
  const r = await fetch(`${base}/properties?page=${page}&limit=${limit}`, {
...
export async function pushEasyBroker(body, externalId, cfg) {
  const { key, base } = ebCfg(cfg);
  if (!key) return { skipped: true };
  const url = externalId ? `${base}/properties/${externalId}` : `${base}/properties`;
...
export async function fetchEBContactRequests(page = 1, cfg) {
  const { key, base } = ebCfg(cfg);
  if (!key) return { skipped: true, leads: [] };
  const r = await fetch(`${base}/contact_requests?page=${page}&limit=50`, {
    headers: { 'X-Authorization': key, accept: 'application/json' },
  });
