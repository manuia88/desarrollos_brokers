...
export const HERRAMIENTAS = [
  ...
  { name: 'agendar_cita', description: 'Agenda la visita cuando el cliente confirme fecha y hora (de horarios_disponibles) y tengas su nombre.',
    input_schema: { type: 'object', properties: {
      ...
      nombre: { type: 'string' }, dev_sku: { type: 'string' } }, required: ['fecha', 'hora', 'nombre'] } },
  { name: 'registrar_prospecto', description: 'Registra al cliente como prospecto en cuanto te diga su nombre (aunque aún no agende).',
    input_schema: { type: 'object', properties: { nombre: { type: 'string' }, interes: { type: 'string', description: 'Qué busca, en una frase' }, dev_sku: { type: 'string' } }, required: ['nombre'] } },
  ...
  async function ejecutar(name, args) {
    ...
    if (name === 'agendar_cita') {
      ...
      const { data: cita, error } = await db.from('citas').insert({
        ...
        nombre: args.nombre, telefono: ctx.canal === 'whatsapp' ? ctx.contacto : null,
    ...
    if (name === 'registrar_prospecto') {
      ...
      const { data: l, error } = await db.from('leads').insert({
        ...
        telefono: ctx.canal === 'whatsapp' ? ctx.contacto : null,
...
export function promptAgente({ lead, nombreOrg }) {
...
export async function responderAgente({ db, ia, orgId, nombreOrg, canal, contacto, texto, historial, lead, asesorId, soloLectura = false }) {
  ...
  const r = await llamarIATools({
    ...
    system: promptAgente({ lead, nombreOrg }),
