...
export const HERRAMIENTAS = [
  ...
  { name: 'consultar_conocimiento', description: 'Consulta la base de conocimiento de la inmobiliaria: políticas (apartado, cancelación, mantenimiento), amenidades a detalle, avance de obra, preguntas frecuentes. Úsala cuando pregunten algo que no está en los datos de precios/unidades.',
    input_schema: { type: 'object', properties: { pregunta: { type: 'string', description: 'La duda del cliente, tal cual' }, dev_sku: { type: 'string', description: 'Desarrollo si la duda es de uno específico' } }, required: ['pregunta'] } },
...
export function ejecutorHerramientas(db, ctx) {
  ...
  async function ejecutar(name, args) {
    ...
    if (name === 'consultar_conocimiento') {
      const { data, error } = await db.rpc('buscar_conocimiento', { p_org: ctx.orgId, p_dev: args.dev_sku || ctx.lead?.dev_sku || null, p_q: String(args.pregunta || '').slice(0, 300), p_max: 4 });
      if (error || !data?.length) return { resultado: 'No hay información sobre eso en la base de conocimiento. Ofrece pasar con un asesor.' };
      return { fragmentos: data.map(x => ({ tema: x.titulo, info: x.texto })) };
    }
    if (name === 'cotizar') {
      let dev = null;
...
- Para políticas (apartado, cancelación, mantenimiento), amenidades a detalle, avance de obra o preguntas frecuentes, usa consultar_conocimiento antes de responder. Si no hay info, dilo y ofrece pasar con un asesor — no inventes.
