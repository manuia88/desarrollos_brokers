...
export async function generarPDF(p) {
  ...
  doc.setTextColor(...ink); F('bold'); doc.setFontSize(22); doc.text(tituloDev(p.dev), M, y); y += 7;
  ...
  doc.save(`Propuesta ${tituloDev(p.dev)}.pdf`);
}
...
export async function generarBrochure(p) {
  ...
  doc.setTextColor(...ink); F('bold'); doc.setFontSize(24); doc.text(tituloDev(dev) || 'Desarrollo', M, y); y += 7;
  F('normal'); doc.setFontSize(11); doc.setTextColor(...sub);
  ...
  doc.save(`Brochure ${tituloDev(dev)}.pdf`);
}
...
export async function generarPPTX(p) {
  ...
  s.addText(tituloDev(p.dev), { x: 8, y: 1.7, w: 4.9, h: 1.1, bold: true, fontSize: 26, color: INK });
  ...
  await pptx.writeFile({ fileName: `Propuesta ${tituloDev(p.dev)}.pptx` });
}
