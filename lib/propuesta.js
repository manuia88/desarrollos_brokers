...
export async function generarPDF(p) {
  ...
  else { doc.setFontSize(13); doc.setTextColor(...ink); F('bold'); doc.text(p.asesor?.org_nombre || 'Quiero Casa', M, 20); }
  ...
  doc.text('Generado con Quiero Casa · Información referencial, sujeta a disponibilidad.', M, H - 8);
...
export async function generarBrochure(p) {
  ...
  doc.setFillColor(...mag); doc.rect(0, 0, W, 4, 'F');
  if (logo) { const lw = 34, lh = Math.min(16, lw * logo.h / logo.w); doc.addImage(logo.data, 'JPEG', M, 12, lw, lh); }
  else { doc.setFontSize(13); doc.setTextColor(...ink); F('bold'); doc.text(asesor.org_nombre || 'Quiero Casa', M, 20); }
  ...
  doc.text('Generado con Quiero Casa · Información referencial, sujeta a disponibilidad.', M, H - 7);
...
export async function generarPPTX(p) {
  ...
  else s.addText(p.asesor?.org_nombre || 'Quiero Casa', { x: 0.5, y: 0.4, w: 6, h: 0.6, bold: true, fontSize: 18, color: INK });
  ...
  s2.addText('Generado con Quiero Casa · Información referencial, sujeta a disponibilidad.', { x: 0.5, y: 7, w: 12, h: 0.3, fontSize: 9, color: SUB });
